...
bool FrontEnd::InitFilter(std::string filter_user, std::shared_ptr<CloudFilterInterface> &filter_ptr,
    ...
    bool FrontEnd::SaveMap() {
        ...
        for(size_t i = 0; i < global_map_frames_.size(); ++i){
            key_frame_path = data_path_ + "/KeyFrame/key_frame_" + std::to_string(i)+ ".pcd";
        ...
        pcl::io::savePCDFile(map_file_path, *global_map_ptr_);
        has_new_global_map_ = true;

        return true;
    }
    bool FrontEnd::GetCurrentScan(CloudData::CLOUD_PTR &current_scan_ptr) {
        display_filter_ptr_->Filter(result_map_ptr_,current_scan_ptr);
        return true;
    }
